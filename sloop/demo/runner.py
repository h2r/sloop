...
class AirSimDemo(Trial):
    ...
    def run(self, logging=False):
        ...
        # Create problem
        mos_problem = mos.MosOOPOMDP("r",
                                     prior=prior,
                                     grid_map=worldstr,
                                     obj_id_map=obj_id_map,
                                     **problem_args)

        init_point_pair   = self._config["airsim_config"]["init_point_pair"]
        second_point_pair = self._config["airsim_config"]["second_point_pair"]
        flight_height     = self._config["airsim_config"]["flight_height"]
        landmark_heights  = self._config["airsim_config"]["landmark_heights"]
        problem = AirSimSearchProblem.from_mos_problem(
            mos_problem,
            mapinfo,
            init_point_pair,
            second_point_pair,
            flight_height,
            map_name=map_name,
            landmark_heights=landmark_heights)
