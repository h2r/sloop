...
class ContextForefModel(BaseModel):
    NAME="context_foref"

    def __init__(self, keyword, learning_rate=1e-4, map_dims=(21,21)):
        super(ContextForefModel, self).__init__()
    ...
    def make_input(cls, data_sample, dataset, mapinfo,
        ...
        return data_sample[FdCtxImg.NAME].float()
    ...
    def Eval(cls, keyword, model, dataset, device, save_dirpath,
        ...
        for kk in range(amount):
            ...
            ctx_map = dataset.rescale(FdCtxImg.NAME, dataset[i][FdCtxImg.NAME])
            test_samples.append(("DummyObject", landmark, map_name, abs_obj_loc, ctx_map))
        rules = {keyword: ForefRule(keyword)}
        rbm_model = RuleBasedModel(rules)
        results = rule_based_evaluate(
            rbm_model, test_samples, mapinfo, keyword,
            map_dims=map_dims,
