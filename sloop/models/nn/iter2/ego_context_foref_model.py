...
class EgoCtxForefAngleModel(BaseModel):
    NAME="ego_ctx_foref_angle"

    def __init__(self, keyword, learning_rate=1e-4, map_dims=(21,21)):
        super(EgoCtxForefAngleModel, self).__init__()
        ...
        self.layer_forigin_input = nn.Sequential(nn.Linear(2, 16),
                                                 ...
                                                 nn.ReLU(),
                                                 nn.Linear(16, 16))
        self.layer_foref = nn.Sequential(nn.Linear(300, FOREF_L1),
                                         nn.ReLU(),
    ...
    def Train(cls, model, trainset, device,
        ...
        return super(EgoCtxForefAngleModel, cls).Train(model, trainset, device,
                                                       criterion=criterion,
                                                       **kwargs)
    ...
    def make_input(cls, data_sample, dataset, mapinfo,
        ...
        inpt = torch.cat([data_sample[FdCtxEgoImg.NAME].float()], 1)
        return inpt.reshape(1,-1)

    def predict_foref(self, keyword, landmark_symbol,
                      map_name, mapinfo, device="cpu",
                      map_img=None, mapsize=(28,28)):
        ...
        if map_img is None:
            ctx_img = ego_lmk_map(landmark_symbol,
                              map_name, mapinfo, mapsize=mapsize,
                              landmark_border=False, use_context=True)
        else:
            ctx_img = map_img
        ctx_img = dummy_dataset.normalize(FdCtxEgoImg.NAME, ctx_img)
        lmk_ctr = mapinfo.center_of_mass(landmark_symbol, map_name)
        prediction_angle = self(torch.tensor(ctx_img).reshape(1,-1).float().to(device))[0].item()
        foref = np.array([*lmk_ctr,
                          dummy_dataset.rescale(FdFoRefAngle.NAME, prediction_angle)], dtype=float)
        return foref
    ...
    def Plot(cls, keyword, model, dataset, device,
        ...
        for i, batch in enumerate(data_loader):
            ...
            foref_pred = np.array([map_width/2, map_length/2,
                                   dataset.rescale(FdFoRefAngle.NAME, prediction_angle)])
            foref_true = np.array([map_width/2, map_length/2,
                                   dataset.rescale(FdFoRefAngle.NAME, batch[FdFoRefAngle.NAME][0])])
            # Record results
            ...
            if i in plot_indices:
                mapimg = batch[FdCtxEgoImg.NAME][0].numpy().reshape(map_dims)
                mapimg = dataset.rescale(FdCtxEgoImg.NAME, mapimg)
