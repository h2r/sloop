...
class EgoBdgForefAngleModel(BaseModel):
    NAME="ego_bdg_foref_angle"

    def __init__(self, keyword, learning_rate=1e-4, map_dims=(21,21)):
        super(EgoBdgForefAngleModel, self).__init__()
    ...
    def Train(cls, model, trainset, device,
        ...
        return super(EgoBdgForefAngleModel, cls).Train(model, trainset, device,
                                                      criterion=criterion,
                                                      **kwargs)
    ...
    def make_input(cls, data_sample, dataset, mapinfo,
        ...
        inpt = torch.cat([data_sample[FdBdgEgoImg.NAME].float()], 1)
    ...
    def predict_foref(self, keyword, landmark_symbol,
        ...
        if map_img is None:
            ...
            bdg_img = map_img
        bdg_img = dummy_dataset.normalize(FdBdgEgoImg.NAME, bdg_img)
    ...
    def Plot(cls, keyword, model, dataset, device,
        ...
        for i, batch in enumerate(data_loader):
            ...
            if i in plot_indices:
                mapimg = batch[FdBdgEgoImg.NAME][0].numpy().reshape(map_dims)
                mapimg = dataset.rescale(FdBdgEgoImg.NAME, mapimg)
                ...
                colors = {"prediction": ("orangered", "orange"),
                          "true": ("lawngreen", "darkkhaki")}
